...
copy_file(Source, Destination) ->
    posix_result(file:copy(Source, Destination)).
...
current_directory() ->
    case file:get_cwd() of
        {ok, Dir} ->
            {ok, path_to_binary(Dir)};
        {error, Reason} ->
            posix_result({error, Reason})
    end.

resolve(Path) ->
    {ok, path_to_binary(filename:absname(Path))}.
...
reason_to_binary(Reason) when is_atom(Reason) ->
    string:uppercase(atom_to_binary(Reason, utf8));
